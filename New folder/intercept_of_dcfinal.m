...
semilogy(gamma_p_values, intercept_probabilities, 'LineWidth', 2);
xlabel('\gamma_{p} (dB)');
...
function intercept_probability = calculateInterceptProbability(gamma_I2, sigma_sp, sigma_ps, lambda_s, a, gamma_p1, Nv)
    ...
    for g = 0:(Nv - 1)
        intercept_probability = intercept_probability + ((2 * (A1^g) * (A1 * sigma_ps)^((1 - g) / 2) / (factorial(g) * sigma_ps)) * besseli(1 - g, 2 * sqrt(A1 / sigma_ps)) - (2 * (A1^g) * ((A1 + A2) * sigma_ps)^((1 - g) / 2) / (factorial(g) * sigma_ps)) * besseli(1 - g, 2 * sqrt((A1 + A2) / sigma_ps)) + (2 * (A3^g) * (A4 / (A3 + 1 / sigma_sp))^((1 + g) / 2) / (factorial(g) * sigma_ps)) * besseli(1 + g, 2 * sqrt(A4 * (A3 + 1 / sigma_sp))));
    end
end
